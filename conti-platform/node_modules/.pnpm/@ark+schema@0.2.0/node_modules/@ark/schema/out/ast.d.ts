...
export type primitiveConstraintKindOf<In> = Extract<PrimitiveConstraintKind, constraintKindOf<In>>;
export declare namespace number {
    ...
    type is<constraints extends Constraints> = of<number, constraints>;
    type constrain<kind extends PrimitiveConstraintKind, schema extends NodeSchema<kind>> = normalizePrimitiveConstraintRoot<schema> extends infer rule ? kind extends "min" ? schema extends {
        exclusive: true;
...
export declare namespace string {
    ...
    type is<constraints extends Constraints> = of<string, constraints>;
    type constrain<kind extends PrimitiveConstraintKind, schema extends NodeSchema<kind>> = normalizePrimitiveConstraintRoot<schema> extends infer rule ? kind extends "minLength" ? schema extends {
        exclusive: true;
...
export declare namespace Date {
    ...
    type is<constraints extends Constraints> = of<Date, constraints>;
    type constrain<kind extends PrimitiveConstraintKind, schema extends NodeSchema<kind>> = normalizePrimitiveConstraintRoot<schema> extends infer rule ? kind extends "after" ? schema extends {
        exclusive: true;
...
export type constrain<t, kind extends PrimitiveConstraintKind, schema extends NodeSchema<kind>> = t extends MorphAst<infer i, infer o> ? (In: leftIfEqual<i, _constrain<i, kind, schema>>) => Out<o> : leftIfEqual<t, _constrain<t, kind, schema>>;
type _constrain<t, kind extends PrimitiveConstraintKind, schema extends NodeSchema<kind>> = schemaToConstraint<kind, schema> extends infer constraint ? parseConstraints<t> extends ([
    infer base,
    infer constraints extends Constraints
]) ? [
    number,
    base
] extends [base, number] ? number.is<constraint & constraints> : [string, base] extends [base, string] ? string.is<constraint & constraints> : [Date, base] extends [base, Date] ? Date.is<constraint & constraints> : of<base, constraints & constraint> : [number, t] extends [t, number] ? number.constrain<kind, schema> : [string, t] extends [t, string] ? string.constrain<kind, schema> : [Date, t] extends [t, Date] ? Date.constrain<kind, schema> : of<t, conform<constraint, Constraints>> : never;
export type parseConstraints<t> = t extends of<infer base, infer constraints> ? equals<t, number & {
    [constrained]: constraints;
}> extends true ? [
    number,
    constraints
...
export type normalizePrimitiveConstraintRoot<schema extends NodeSchema<PrimitiveConstraintKind>> = "rule" extends keyof schema ? conform<schema["rule"], PropertyKey> : conform<schema, PropertyKey>;
export type schemaToConstraint<kind extends PrimitiveConstraintKind, schema extends NodeSchema<kind>> = normalizePrimitiveConstraintRoot<schema> extends infer rule ? kind extends "pattern" ? Matching<rule> : kind extends "divisor" ? DivisibleBy<rule> : kind extends "exactLength" ? Length<rule> : kind extends "min" ? schema extends {
    exclusive: true;
} ? MoreThan<rule> : AtLeast<rule> : kind extends "max" ? schema extends {
    exclusive: true;
...
type _distill<t, io extends "in" | "out", distilledKind extends "base" | "constrainable"> = t extends undefined ? t : [t] extends [anyOrNever] ? t : parseConstraints<t> extends ([
    infer base,
    infer constraints extends Constraints
